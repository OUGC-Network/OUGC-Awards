...
if($awards->get_input('action') == 'add' || $awards->get_input('action') == 'edit')
{
	...
	$form_container->output_row($lang->ougc_awards_form_additionalgroups, $lang->ougc_awards_form_additionalgroups_desc, $form->generate_yes_no_radio('additionalgroups', $mybb->input['additionalgroups']));
	...
	$form_container->output_row($lang->ougc_awards_form_requirements_previousawards, $lang->ougc_awards_form_requirements_previousawards_desc, $awards->generate_awards_select('previousawards[]', $mybb->input['previousawards'], array('multiple' => true)));
	...
	if($reputation_select_r = $awards->generate_ougc_custom_reputation_select('ougc_customrepids_r', $awards->get_input('ougc_customrepids_r', 1)))
	{
...
elseif($awards->get_input('action') == 'logs')
{
	...
	{
		$table->construct_cell('<div align="center">'.$lang->ougc_awards_logs_empty.'</div>', array('colspan' => 5));
		$table->construct_row();
	...
	else
	{
		...
		while($log = $db->fetch_array($query))
		{
			...
			$table->construct_cell('<a href="'.$awards->build_url(array('action' => 'edit', 'tid' => $log['tid'])).'">'.$task['name'].'</a>');
...
elseif($awards->get_input('action') == 'users')
{
	...
	else
	{
		...
		echo draw_admin_pagination($awards->get_input('page', 1), $awards->query_limit, $givedscount, 'index.php?module=user-ougc_awards&amp;view=category&amp;cid='.$cid.'&amp;action=users&amp;aid='.$award['aid']);
...
elseif($awards->get_input('action') == 'owners')
{
	...
	else
	{
		...
		echo draw_admin_pagination($awards->get_input('page', 1), $awards->query_limit, $givedscount, 'index.php?module=user-ougc_awards&amp;view=category&amp;cid='.$cid.'&amp;action=owners&amp;aid='.$award['aid']);
