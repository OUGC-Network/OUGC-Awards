...
function ougc_awards_info()
{
    ...
    return array(
        'name' => 'OUGC Awards',
        'description' => $lang->setting_group_ougc_awards_desc . ($awards->allow_imports && ougc_awards_is_installed(
            ) ? $lang->ougc_awards_import_desc : ''),
        'website' => 'https://ougc.network',
        'author' => 'Omar G.',
...
function ougc_awards_activate()
{
    ...
    find_replace_templatesets(
        ...
        '{$post[\'user_details\']}{$post[\'ougc_awards\']}{$post[\'ougc_awards_preset\']}'
    );
    find_replace_templatesets(
        'postbit_classic',
        '#' . preg_quote('{$post[\'user_details\']}') . '#',
    ...
    find_replace_templatesets(
        ...
        '{$warning_level}{$memprofile[\'ougc_awards_preset\']}'
    );
    find_replace_templatesets(
        'modcp_nav_users',
        '#' . preg_quote('{$nav_ipsearch}') . '#',
        '{$nav_ipsearch}<!--OUGC_AWARDS-->'
    );
    find_replace_templatesets(
        'usercp_nav_misc',
        '#' . preg_quote('{$lang->ucp_nav_view_profile}</a></td></tr>') . '#',
        '{$lang->ucp_nav_view_profile}</a></td></tr><!--OUGC_AWARDS-->'
    );
    find_replace_templatesets(
        'modcp_nav',
        '#' . preg_quote('mcp_nav_editprofile}</a></td></tr>') . '#',
        'mcp_nav_editprofile}</a></td></tr><!--OUGC_AWARDS-->'
    ); // 1.6
    ...
    find_replace_templatesets(
        'stats',
        '#' . preg_quote('{$footer}') . '#',
        '{$ougc_awards_most}{$ougc_awards_last}{$footer}'
    );
    find_replace_templatesets(
        'headerinclude',
        '#' . preg_quote('{$stylesheets}') . '#',
        '{$stylesheets}{$ougc_awards_js}{$ougc_awards_css}'
    );
    find_replace_templatesets(
        'header_welcomeblock_member',
        '#' . preg_quote('{$searchlink}') . '#',
        '{$ougc_awards_welcomeblock}{$searchlink}'
    );
...
function ougc_awards_deactivate()
{
    ...
    find_replace_templatesets(
        'postbit_classic',
        '#' . preg_quote('{$post[\'{$post[\'ougc_awards_preset\']}\']}') . '#',
        '',
        0
    );
    ...
    find_replace_templatesets(
        'header_welcomeblock_member',
        '#' . preg_quote('{$ougc_awards_welcomeblock}') . '#',
        '',
        0
    );
...
function ougc_awards_pl_check()
{
    ...
    if (!file_exists(PLUGINLIBRARY)) {
        flash_message(
            $lang->sprintf($lang->ougc_awards_pl_required, $info['pl']['url'], $info['pl']['version']),
            'error'
        );
    ...
    if ($PL->version < $info['pl']['version']) {
        flash_message(
            $lang->sprintf($lang->ougc_awards_pl_old, $info['pl']['url'], $info['pl']['version'], $PL->version),
            'error'
        );
        admin_redirect('index.php?module=config-plugins');
        exit;
...
function ougc_awards_modcp()
{
    ...
    if ($modcp) {
        $permission = (bool)($mybb->settings['ougc_awards_modcp'] && ($mybb->settings['ougc_awards_modgroups'] == -1 || ($mybb->settings['ougc_awards_modgroups'] && $awards->check_groups(
        ...
        if ($permission) {
            ...
            $awards_nav = eval($templates->render('ougcawards_modcp_nav'));
        ...
        $awards_nav = eval($templates->render('ougcawards_usercp_nav'));
    ...
    if ($awards->get_input('manage') == 'requests') {
        ...
        if ($awards->get_input('view') == 'logs') {
            add_breadcrumb(
                $lang->ougc_awards_modcp_requests_list_viewlogs,
                $awards->build_url(['manage' => 'requests', 'view' => 'logs'])
            );
            $status = "!='1'";
        }
        ...
        if ($mybb->request_method == 'post') {
            ...
            if (empty($errors)) {
                $query = $db->simple_select(
                    ...
                    '*',
                    "status{$status} AND aid IN ('{$where_aids}') AND rid IN ('" . implode(
                        "','",
                        array_keys($selected_list)
                    ) . "')"
                );
        ...
        $query = $db->simple_select(
            'ougc_awards_requests',
            '*',
            "status{$status} AND aid IN ('{$where_aids}')",
            array('limit_start' => $start, 'limit' => $awards->query_limit)
        );
        if (!$db->num_rows($query)) {
            $requestslist = eval($templates->render('ougcawards_modcp_requests_list_empty'));
        } else {
            while ($request = $db->fetch_array($query)) {
                $requests[] = $request;
            ...
            $multipage = (string)multipage(
                ...
                $awards->build_url(
                    ($awards->get_input('view') != 'logs' ? 'manage=requests' : array(
                        'manage' => 'requests',
                        'view' => 'logs'
                    ))
                )
            ...
            foreach ($requests as $request) {
                ...
                $requestslist .= eval($templates->render('ougcawards_modcp_requests_list_item'));
                $trow = alt_trow();
                unset($award);
            }

            $awards->get_input('view') == 'logs' or $buttons = eval(
            $templates->render(
        ...
        $lang->ougc_awards_modcp_title_give = $lang->sprintf(
            $lang->ougc_awards_modcp_title_give,
            $awards->get_award_info('name', $award['aid'])
        );

        $gived_list = '';
        $multiple = eval($templates->render('ougcawards_modcp_manage_multiple'));
        $username = eval($templates->render('ougcawards_modcp_manage_username'));
        $reason = eval($templates->render('ougcawards_modcp_manage_reason'));
        $thread = eval($templates->render('ougcawards_modcp_manage_thread'));
        $content = eval($templates->render('ougcawards_modcp_manage'));
    ...
    elseif ($awards->get_input('manage') == 'revoke') {
        ...
        if ($show_gived_list) {
            ...
            $username = eval($templates->render('ougcawards_modcp_manage_username_hidden'));
            $gived_list = eval($templates->render('ougcawards_modcp_manage_gived_list'));
        } else {
            $username = eval($templates->render('ougcawards_modcp_manage_username'));
        }
        ...
        $query = $db->query(
			...
			WHERE u.uid='" . (int)$mybb->user['uid'] . "' AND a.visible='1' AND a.cid IN ('" . implode(
                "','",
                array_values($cids)
            ) . "')
            ...
            foreach ($categories as $cid => $category) {
                ...
                if (empty($_awards[(int)$category['cid']])) {
                    ...
                    foreach ($_awards[(int)$category['cid']] as $cid => $award) {
                        ...
                        $awardlist .= eval($templates->render('ougcawards_usercp_sort_award'));
                        $trow = alt_trow();
                    }
                }

                $content .= eval($templates->render('ougcawards_usercp_sort'));
            }
        }
        ...
        if ($pid) {
            ...
            $query = $db->query(
				...
				LEFT JOIN {$db->table_prefix}ougc_awards a ON (u.aid=a.aid)
				LEFT JOIN {$db->table_prefix}users ou ON (u.oid=ou.uid)
				WHERE u.uid='{$mybb->user['uid']}' AND a.visible='1' AND a.cid IN ('" . implode(
                    "','",
                    array_values($cids)
                ) . "')
				ORDER BY u.disporder, u.date desc"
        ...
        if ($db->num_rows($query)) {
            ...
            $multipage = (string)multipage(
                ...
                $awards->query_limit,
                $awards->get_input('page', 1),
                $awards->build_url($modcp ? '' : 'manage=default')
            );
        ...
        foreach ($categories as $cid => $category) {
            ...
            if (empty($cached_items[(int)$cid])) {
                ...
                foreach ($cached_items[(int)$cid] as $cid => $award) {
                    ...
                    $award['fimage'] = eval($templates->render($awards->get_award_info('template', $award['aid'])));
                    $awardlist .= eval($templates->render('ougcawards_modcp_list_award'));
                }
            }

            $content .= eval($templates->render('ougcawards_modcp_list'));
        }
    ...
    $page = eval($templates->render('ougcawards_modcp'));
    output_page($page);
    exit;
}
...
function ougc_awards_profile()
{
    ...
    $whereclause = "u.visible=1 AND u.uid='" . (int)$memprofile['uid'] . "' AND a.visible='1' AND a.type!='2' AND a.cid IN ('" . implode(
            "','",
            array_values($cids)
        ) . "')";
    ...
    if ($display_awards && $awardscount && $awards->query_limit_profile > 0) {
        ...
        $multipage = (string)multipage(
            $awardscount,
            $awards->query_limit_profile,
            $page,
            $awards->build_url('view=awards')
        );
    ...
    if (!$db->num_rows($query)) {
        ...
        if ($display_awards) {
            ...
            if (!empty($_awards)) {
                ...
                //$awardlist .= eval($templates->render('ougcawards_profile_row_category'));
                ...
                foreach ($_awards as $award) {
                    ...
                    $award['fimage'] = eval($templates->render($awards->get_award_info('template', $award['aid'])));
                    $awardlist .= eval($templates->render('ougcawards_profile_row'));
                    $trow = alt_trow();
    ...
    if ($display_awards) {
        $lang->ougc_awards_profile_title = $lang->sprintf(
            $lang->ougc_awards_profile_title,
    ...
    foreach ($awardlist as $award) {
        ...
        if ($awards->query_limit_postbit == -1 || $count < $awards->query_limit_postbit) {
            ...
            if ($awards->query_limit_postbit != -1 && $count == $awards->query_limit_postbit && $total != $count) {
                ...
                $viewall = eval($templates->render('ougcawards_stats_user_viewall'));
            }
            ...
            $new_award = eval($templates->render('ougcawards_postbit', true, false));
...
function ougc_awards_global_intermediate()
{
    ...
    if ($mybb->settings['ougc_awards_welcomeblock']) {
        $ougc_awards_welcomeblock = eval($templates->render('ougcawards_welcomeblock'));
    }
    ...
    $ismod = ($mybb->usergroup['canmodcp'] && $mybb->settings['ougc_awards_modcp'] && ($mybb->settings['ougc_awards_modgroups'] == -1 || $awards->is_member(
                $mybb->settings['ougc_awards_modgroups']
            )));
...
class OUGC_Awards
{
    ...
    public function _db_verify_indexes()
    {
        ...
        if ($db->index_exists('ougc_awards_users', 'uidaid')) {
            //$db->write_query('ALTER TABLE '.$db->table_prefix.'ougc_awards_users ADD UNIQUE KEY uidaid (uid,aid)');
            $db->drop_index('ougc_awards_users', 'uidaid');
        }
        if ($db->index_exists('ougc_awards_users', 'aiduid')) {
            //$db->write_query('CREATE INDEX aiduid ON '.$db->table_prefix.'ougc_awards_users (aid,uid)');
            $db->drop_index('ougc_awards_users', 'aiduid');
        }
    }

    // Install/update task file
    public function update_task_file($action = 1)
    {
        global $db, $lang;
        $this->lang_load();
    ...
    public function lang_load($extra = false)
    {
        global $lang;
    ...
    // $PL->is_member(); helper
    public function is_member($gids, $user = false)
    {
        global $PL;
        $PL or require_once PLUGINLIBRARY;
    ...
    public function update_cache()
    {
        ...
        if ($cids = array_keys($_cache['categories'])) {
            ...
            $query = $db->simple_select(
                'ougc_awards',
                'aid, cid, name, template, description, image, allowrequests, type',
        ...
        if ($aids = array_keys($_cache['awards'])) {
            ...
            $query = $db->simple_select(
                ...
                'uid, date',
                $where,
                array('order_by' => 'date', 'order_dir' => 'desc', 'limit' => $limit)
            );
            while ($user = $db->fetch_array($query)) {
    ...
    // Modify acp url
    public function set_url($params, $url = false)
    {
        if ($url !== false) {
            $this->url = $url;
        }
    ...
    // Get the rate icon
    public function get_award_icon($aid)
    {
    ...
    public function set_cache($type, $id, $data)
    {
    ...
    public function get_request($rid)
    {
    ...
    public function insert_award($data, $aid = null, $update = false)
    {
        global $db;
    ...
    public function update_award($data, $aid)
    {
        $this->insert_award($data, $aid, true);
    }

    // Insert a new request to the DB
    public function insert_request($data, $rid = null, $update = false)
    {
        global $db;
    ...
    public function update_request($data, $rid)
    {
        $this->insert_request($data, $rid, true);
    }

    // Insert a new category to the DB
    public function insert_category($data, $cid = null, $update = false)
    {
        global $db;
    ...
    public function insert_task($data, $tid = null, $update = false)
    {
        global $db;
        ...
        !isset($data['requirements']) or $cleandata['requirements'] = $db->escape_string(
            implode(',', array_filter(array_unique((array)$data['requirements'])))
        );
    ...
    // Redirect admin help function
    public function redirect($message = '', $title = '')
    {
    ...
    public function get_gived_award($aid, $uid, $gid = 0)
    {
        global $db;
    ...
    // Give an award.
    public function give_award($award, $user, $reason, $thread = 0, $task = 0, $rid = 0)
    {
        ...
        $this->send_pm(array(
            ...
            'message' => $lang->sprintf(
                ...
                $user['username'],
                $award['name'],
                (!empty($reason) ? $reason : $lang->ougc_awards_pm_noreason),
                $this->get_award_icon($award['aid']),
    ...
    // Grant owner status.
    public function insert_owner($award, $user)
    {
        global $db, $plugins;
    ...
    // Grant owner status.
    public function revoke_owner($oid)
    {
        global $db, $plugins;
    ...
    public function rebuild_owners($uid = 0)
    {
        global $db;
    ...
    // Grant owner status.
    public function get_owner($aid, $uid, $oid = 0)
    {
        global $db;
    ...
    public function accept_request($rid)
    {
        global $lang, $mybb;
        $this->lang_load();
    ...
    public function reject_request($rid)
    {
        global $lang, $mybb;
        $this->lang_load(true);
        ...
        $this->send_pm(array(
            'subject' => $lang->sprintf(
                $lang->ougc_awards_pm_noreason_request_rejected_subject,
                strip_tags($award['name'])
            ),
            'message' => $lang->sprintf(
                $lang->ougc_awards_pm_noreason_request_rejected_message,
                $user['username'],
                strip_tags($award['name'])
            ),
            'touid' => $user['uid']
    ...
    public function get_award_info($type, $aid, $gid = 0, $rid = 0, $tid = 0)
    {
        global $lang, $cache;
        $this->lang_load(true);
        $aid = (int)$aid;
    ...
    // Revoke an award.
    public function revoke_award($gid)
    {
        global $db, $plugins;
    ...
    public function delete_award($aid)
    {
        global $db;
        $this->aid = (int)$aid;
    ...
    public function update_gived($gid, $data)
    {
        global $db, $plugins;
        $this->gid = (int)$gid;
    ...
    public function parse_text(&$message)
    {
        global $parser;
        if (!is_object($parser)) {
            require_once MYBB_ROOT . 'inc/class_parser.php';
    ...
    public function check_groups($groups, $empty = true)
    {
    ...
    public function can_edit_user($uid)
    {
        global $mybb;
        $uid = (int)$uid;
    ...
    // Insert an alert
    public function send_alert($type = 'give_award')
    {
        ...
        $query = $db->simple_select(
            'alerts',
            'id',
            "object_id='{$this->aid}' AND uid='{$this->uid}' AND unread=1 AND alert_type_id='{$alertType->getId()}'"
        );
    ...
    public function get_thread_by_url($threadurl)
    {
        global $db, $mybb;
     ...
     */
    public function get_input($name, $type = 0)
    {
        global $mybb;
    ...
    public function generate_awards_select($name, $selected = array(), $options)
    {
        global $db, $mybb;
    ...
    public function generate_profilefields_select($name, $selected = array(), $options)
    {
        global $db, $mybb;
    ...
    public function generate_gived_select($aid, $uid, $input)
    {
        global $db, $mybb;
        ...
        while ($gived = $db->fetch_array($query)) {
            ...
            if ($mybb->version_code >= 1800) {
                ...
                $date = my_date($mybb->settings['dateformat'], $gived['date']) . ' - ' . my_date(
            ...
            $select .= "<option value=\"{$gived['gid']}\"{$selected}>" . $date . ' (' . htmlspecialchars_uni(
                    $gived['reason']
                ) . ')</option>';
        }
    ...
    public function generate_category_select($aid, $input)
    {
        global $db, $mybb;
    ...
    public function generate_ougc_custom_reputation_select($name, $selected = 0)
    {
        global $db, $mybb;
    ...
    public function update_preset($data, $pid)
    {
        global $db;
    ...
    public function get_preset($pid)
    {
        global $db;
    ...
    public function delete_preset($pid)
    {
        global $db;
    ...
    public function parse_presets(&$preset_options, $presets_cache, $pid)
    {
        global $templates;
    ...
    public function parse_postbit_awards(&$_awards, $start)
    {
